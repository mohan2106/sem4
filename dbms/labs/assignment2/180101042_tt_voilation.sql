...
BEGIN
    ...
    FROM (SELECT C1.roll, C1.name, C1.cid , C2.exam_date,C2.start_time
        FROM cwsl as C1 JOIN ett as C2 
        ON C1.cid = C2.cid) AS T1
        JOIN 
        (SELECT C1.roll, C1.name, C1.cid , C2.exam_date,C2.start_time
        FROM cwsl as C1 JOIN ett as C2 
        ON C1.cid = C2.cid) AS T2
        WHERE T1.roll = T2.roll and
         T1.exam_date=T2.exam_date and T1.start_time=T2.start_time 
        and T1.cid <> T2.cid;
    ...
    LOOP1: LOOP
        ...
        name=t_name and fcid=cid_2 and scid=cid_1);
        IF num = 0 THEN
